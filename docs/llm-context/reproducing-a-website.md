...
4. **Walk each page's modules.** For each `perPagePlan` entry, iterate its `suggestedModuleTypes` and call `add_module` to insert each one. Then `set_module_content` for every module:
   ...
   - `image-gallery` populates `items[]` one entry per asset, where each entry is `{ image: <assetRef>, caption?: <string> }`. Pull captions from `extractedContent` if present (figcaption-adjacent text); otherwise leave `caption` unset rather than fabricating one. Set the `imageSize` dial (`sm` / `md` / `lg`) to match how prominent images appear in the source — `sm` for thumbnail-style galleries, `md` for standard, `lg` when the source treats images as the main subject. Only the `masonry` variant uses this cap; `grid` is fixed 1:1 squares and ignores it.
