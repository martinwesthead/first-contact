...
updated_at: '2026-06-28T22:11:57.488219+00:00'
completed_at: null
...
**As the** 1st Contact platform operator,
**I want** the AI to receive a structured result after every tool call and to be able to read the current draft definition back on demand, my assistant's replies rendered as formatted markdown (not raw text), a chat input that understands pasted markdown, structured tool outcomes shown as consistent cards in the chat log, and the Send button to lock into a busy state while a turn is still running,
**so that** the AI reasons from confirmed state rather than its own inference across a multi-step change, the chat is legible and visually consistent, and I cannot accidentally fire a second send that races the response in flight.
...
- The chat input is a rich-text editor that accepts pasted markdown and shows it as formatted text, sends on Cmd/Ctrl+Enter, and serializes its content back to markdown for the `/api/chat` request. The input row lays the editor and Send button out as `[ editor ][ Send ]`: the editor renders as a visible bordered, dark-background field that fills the available width and highlights its border on focus, while the Send button stays compact on its right. While a chat turn is in flight — between the Send click / Cmd+Ctrl+Enter submit and the moment `onSend` settles — the Send button is disabled and swaps its "Send" label for a CSS-only spinner with `aria-busy="true"`; repeat clicks and repeat Cmd/Ctrl+Enter are suppressed (an empty input still no-ops before the busy state is entered), the editor itself stays editable so the operator can compose the next message, and the button resets to "Send" and re-enables when the turn settles whether `onSend` resolves or rejects.
...
- **In-flight send-blocking (REQ-32)**: the chat panel owns a private `busy` flag (panel-local, not store-backed) tracking the `onSend` promise it last dispatched. `setBusy(true)` toggles the Send button's `disabled` property and adds `aria-busy="true"` plus a `data-fc-chat-send-busy` attribute; `apps/control-app/public/builder.html` hides the `.fc-chat__send-label` span and reveals a CSS-only rotating `.fc-chat__send-spinner` (border + `@keyframes fc-chat-spin`, no SVG/asset dependency), and the button keeps a min-width so the row does not reflow when the spinner appears. `submit()` returns early while `busy` is set, and an empty input no-ops before the busy state is entered; the `keydown` handler likewise no-ops Cmd/Ctrl+Enter while busy. A `finally` clears the flag whether `onSend` resolves or rejects, and the dispatched promise is `.catch`-guarded so a rejected `onSend` does not surface as an unhandled rejection on the page. The editor is never disabled, so the operator can keep typing the next message while a turn runs.
