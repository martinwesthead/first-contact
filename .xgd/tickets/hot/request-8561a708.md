...
updated_at: '2026-06-19T22:07:48.306894+00:00'
completed_at: null
...
- **Multi-page plan source.** The digest's `perPagePlan` is derived from the sitemap discovered by [[REQ-21]] / [[REQ-22]]. Each entry carries `{ url, slug, title, screenshotKey, extractedContent[], suggestedModuleTypes[] }`. `suggestedModuleTypes` is a heuristic hint (e.g. presence of a form → contactForm; large hero img + headline → hero) computed deterministically from layout signals — no LLM.
...
- **New type** `TranscriptionDigest` (alongside existing digest types from [[REQ-21]]):
  ...
  type TranscriptionDigest = {
    ...
    perPagePlan: Array<{
      ...
      screenshotKey: string;                // R2 key of the desktop screenshot
    ...
    assetInventory: Array<{
      sourceUrl: string;
      r2Key: string;                        // present even before mirror completes? no — see below
...
- **New** `buildTranscriptionDigest(digest, perPageSignals, mirrorSummary)` — pure function that assembles a `TranscriptionDigest` from the Layer-A digest + per-page extraction + mirror results.
- **New** `extractPageContent(html)` → `ExtractedBlock[]` — DOM-driven extraction of headings (`h1`–`h6`), paragraphs, list items, anchor text, form field labels. Deterministic.
- **New** `inferSuggestedModuleTypes(signals)` → `string[]` — pure heuristic. Forms present → `contactForm`. Large hero img + headline → `hero`. Anchor-list nav → `header`. Etc. Order matters: top-down by suggested page composition.
...
- **`transcribe-site.ts` reshape** (`apps/control-app/src/operator/transcribe-site.ts`):
  ...
  - Stage 3 becomes: `buildTranscriptionDigest(...)` → write to R2 at `sites/{siteId}/transcription/digest.json`.
  - Stage 4 (asset mirror) runs inline before digest write so the digest's `assetInventory[].r2Key` values are populated.
...
- **New action** `read_transcription_digest` (category: `system_action`):
  ...
  - Error mode: `{ ok: false, error: 'digest_not_found' }` when the key is missing.
- **New state-edit actions** for page CRUD (per [[REQ-14]] §Page management, implemented here):
  - `add_page` — input `{ slug, title, after_slug? }`. Inserts a new page with empty modules and default seoMeta. Validates slug per [[REQ-10]]'s `isValidSlug`, uniqueness, and that `after_slug` exists if provided.
...
- **Chat system prompt** (`apps/control-app/src/chat.ts`'s `buildSystemPrompt`):
  - Append the contents of `docs/llm-context/reproducing-a-website.md` to the system prompt (read at chat-loop init from the deployed worker's bundled assets, or via a constant import). Not concatenated per turn from the operator side.
...
Reuse [[REQ-28]]'s `tests/fixtures/convert-flow/assets-heavy/` and `duplicate-asset/`. No new fixtures.

## OUT (explicitly deferred)

- Nav-editing tools (`set_nav_pattern`, `set_nav_entries`) — stay in [[REQ-14]] backlog.
- `duplicate_module` — stays in [[REQ-14]] backlog.
- Asset-mirror race fix — Stage 4 now runs inline so the digest is internally consistent; the previously-noted "external URLs render before R2 swap" race no longer applies in this design.
...
- [[REQ-10]] — `isValidSlug` for `add_page`.
...
3. **`read_transcription_digest` returns the digest.** A call against the just-written key returns the same JSON. A call against a missing key returns `{ ok: false, error: 'digest_not_found' }`.
...
7. **Per-page plan present.** Against `tests/fixtures/convert-flow/assets-heavy/`, the digest's `perPagePlan` has at least one entry; against a multi-page fixture (new sub-fixture added if not present in REQ-28's set), `perPagePlan` has multiple entries with distinct `slug` values.
...
9. **End-to-end killer demo.** UAT: convert `tests/fixtures/convert-flow/assets-heavy/` end-to-end via the chat loop (model: Opus 4.7). The resulting working draft has (a) ≥1 page; (b) theme tokens applied from the digest's `themeTokens` (palette + typography differ from 1stcontact defaults); (c) at least one module whose image content field resolves to `/assets/sites/{siteId}/imports/…`; (d) at least one module whose text content contains a string present in the fixture's extracted content. Test asserts each property.
10. **Multi-page killer demo.** UAT: a new multi-page fixture (`tests/fixtures/convert-flow/multi-page/` — minimum 3 HTML pages linked from a header) converts successfully and the resulting draft has ≥3 pages with distinct slugs.
...
8. Transcribe-site handler reshape + extractor function refactor + 4 new operator actions (1 system_action, 3 state_edit) + new LLM-context doc + chat-prompt wiring + 2 new UAT fixtures + ~15 UATs.
...
- Existing REQ-28 implementation has Stage-4 sync-await (per its "Deviations" note). REQ-30 keeps it inline, which is consistent.
