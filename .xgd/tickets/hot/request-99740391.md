...
updated_at: '2026-06-25T02:40:55.910527+00:00'
completed_at: '2026-06-25T02:40:55.910527+00:00'
...
fields:
  ...
  - 927bf47048e901a31dbabc2932d37c46d079ffed
...
## Polish iteration (post-bundle, session 2026-06-15)

Two user-reported defects after first walkthrough of the live preview:

1. **Collapsed chat panel had no restore affordance.** The chevron collapsed the chat panel, but the restore bar ended up on the right edge of the screen instead of the left — because the bar was appended after the preview panel in DOM order. The user said "there's no column to bring it back". Mirrors the XGD Intent Tab pattern (`#intents-list-panel.minimized` → 32px container with a full-height `»` button on the left).
2. **Preview iframe collapsed to ~50px when viewport preset selected.** The `.fc-preview` wrapper had no display rule, so the iframe's `height:100%` resolved against an auto-height parent block and collapsed to its intrinsic ~150px (toolbar pushed the visible iframe down further).

### Fix

- `packages/builder-ui/src/components/builder-layout.ts` — insert `restoreBar` BEFORE `previewPanel` in DOM order; add inline flex-centering + `height:100%` so it renders as a visible left rail even without page CSS.
- `packages/builder-ui/src/components/preview-panel.ts` — set inline flex column sizing on `.fc-preview` root (display:flex, flex:1 1 auto, min-height:0, height:100%) and `flex:1 1 auto; min-height:0` on the iframe so the toolbar gets natural height and the iframe takes the remainder.
- `apps/control-app/public/builder.html` — back the JS choices with matching CSS rules (`.fc-builder__restore` full-height flex-centered; `.fc-preview` flex column; `.fc-preview__iframe` min-height:0).

### New UATs

- `test_UAT_FC_REQ-8_restore_rail_left_of_preview` — asserts DOM ordering (restoreBar before previewPanel) and inline flex styling on the rail.
- `test_UAT_FC_REQ-8_preview_fills_panel_height` — asserts `.fc-preview` root and `.fc-preview__iframe` carry the flex sizing contract through a viewport switch.

All 10 REQ-8 UATs pass (15 tests).

### Commit lineage note

- Original bundle commit: `0b1ba59f`.
- Polish commit: `927bf47` — carries the actual code changes for these fixes. The auto-quality-apply background process produced the commit and used its own subject ("xgd(quality): apply ai.gendevlabs.javascript_vitest_open to quality.yaml"); the `[FREE-CODED]` tag is therefore NOT in that commit's message. A subsequent commit `1a0e901` reasserts the [FREE-CODED] message intent but contains only an auto-generated quality report. Operator should be aware before merge-back review.